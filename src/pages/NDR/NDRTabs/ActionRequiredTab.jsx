...
import { Button, DatePicker, Input, message, Popover, Space, Table, Tag } from 'antd';
import { MenuFoldOutlined, SearchOutlined, ThunderboltOutlined } from '@ant-design/icons';
...
console.log(payload);
  ...
  const columns = [
    {
      ...
      render: (text, order) => (
        <Link style={{ color: 'black', fontWeight: '400', fontFamily: 'Poppins', textAlign: 'center' }} to={`/orders/In/updateorder/${order?._id}/${order?.orderId}`}>
          {order.orderId}
        </Link>
      ),
    ...
    },
    {
      ...
      render: (text, order) => (
        ...
        {order.shippingPartner && order.awb &&  <a target='_blank' href={`/tracking/shipment/${order.shippingPartner}/${order.awb}`}>
  ...
  const ndrOrders = dataSource?.filter((order) => order?.status === 'UnDelivered')
  return (
    <div>
        ...
        <Popover
          ...
          title={
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <Button onClick={() => { setIsDatePickerOpen(true); }} loading={loading}>
                Re-attempt
              </Button>
              <Button onClick={() => handleAction('RTO')} loading={loading}>
                RTO
              </Button>
              <Button onClick={() => handleAction('Reschedule')} loading={loading}>
                Reschedule
              </Button>
            </div>
          }
