...
export const DeliveryPartnerProvider = ({ children }) => {
    ...
    useEffect(() => {
        const fetchDeliveryPartners = async () => {
            try {
                ...
                const response = await fetch('http://localhost:5000/api/shipping/getDeliveryPartners', {
                    headers: {
                        Authorization: `${token}`,
                    },
                });
    ...
    return (
        <DeliveryPartner.Provider value={{ deliveryPartners, setDeliveryPartners, loading, error }}>
            {children}
